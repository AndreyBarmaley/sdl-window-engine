...
namespace SWE
{
    ...
    void WindowListBox::setSelectedItemIndex(int index)
    {
        ...
        else if(0 > index && index != indexSelected)
        {
            indexSelected = -1;
            signalEmit(Signal::ListboxSelectedItem);
            renderWindow();
    ...
    bool WindowScrollBar::mousePressEvent(const ButtonEvent & be)
    {
        ...
        else if(be.isButtonLeft() && (scrollArea() & be.position()))
        {
            ...
            else if(! isVertical() && pos1.x > localArea.w - cursz)
    ...
    void WindowButton::renderWindow(void)
    {
        ...
        if(isDisabled())
            ...
            tx = textureFocused();
        else if(checkState(FlagButtonInformed) && textureInformed() && textureInformed()->isValid())
            tx = textureInformed();
        else
            tx = textureReleased();
    ...
    WindowTextArea & WindowTextArea::appendString(const FontRender & frs, const UCString & str, int halign, bool wrap)
    {
        ...
        else if(str.length())
        {
            ...
            {
                case AlignRight:
                    offset.x = width() - tx.width();
                    break;

                case AlignCenter:
                    offset.x = (width() - tx.width()) / 2;
                    break;
