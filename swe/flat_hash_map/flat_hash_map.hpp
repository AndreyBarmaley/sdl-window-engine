...
struct prime_number_hash_policy
{
    ...
    mod_function next_size_over(uint64_t & size) const
    {
        ...
        static constexpr uint64_t prime_list[] =
        {
        ...
        auto found = std::lower_bound(std::begin(prime_list), std::end(prime_list) - 1, size);
        size = *found;
...
struct fibonacci_hash_policy
{
	...
	const uint64_t two = 2;
        size = std::max(two, detailv3::next_power_of_two(size));
