...
public:
    ...
    size_t              lastg(void) const { return sr.last(); }
    size_t              tellg(void) const { return sr.tell(); }
    bool                seekg(size_t pos) { return sr.seek(pos); }
    bool                skipg(size_t len) { return sr.skip(len); }
...
public:
    ...
    bool                seekp(size_t pos) { return sw.seek(pos); }
    bool                skipp(size_t len) { return sw.skip(len); }

    const u8*           data(void) const { return buf.data() + tellg(); }
    size_t              size(void) const { return lastg(); }
